"""test_cylinder.py - test cylinder grid and methods
Copyright 2022 by Eric Conrad

USAGE

    usage: test_cylinder.py [-h] [-d DIM DIM]

    Test cylindrical grid and fundamental maze algorithms.
...
import mazelib
from cylinder_grid import CylinderGrid, BinaryTree, CocktailShaker, \
    Sidewinder
...
if __name__ == '__main__':
    ...
    desc = 'Test cylindrical grid and fundamental maze algorithms.'
    ...
    rows, cols = args.dim
        ...
        # --------- TESTING STARTS HERE ----------
    print('Test 1)', 'lazy binary tree on a cylindrical grid')
    grid = CylinderGrid(rows, cols)
    maze = Maze(grid)
    BinaryTree.on(maze, mark_split=True)
    print(maze)
    ...
    print('Test 2)', 'cocktail shaker binary tree on a cylindrical grid')
    grid = CylinderGrid(rows, cols)
    maze = Maze(grid)
    CocktailShaker.on(maze, mark_split=True)
    print(maze)
    chi = maze_characteristic(maze)
    if maze.k > 1:
        print('There are some isolated cells in the maze.')
    ...
    print('Test 3)', 'sidewinder tree on a cylindrical grid')
    grid = CylinderGrid(rows, cols)
    maze = Maze(grid)
    Sidewinder.on(maze, mark_split=True)
    print(maze)
    chi = maze_characteristic(maze)
    if maze.k > 1:
        print('There are some isolated cells in the maze.')
    ...
    print('Test 4)', 'Euler characteristic of the cylindrical grid')
