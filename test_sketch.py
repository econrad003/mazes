"""test_sketch.py - test the implementation of sketching
Copyright 2022 by Eric Conrad

USAGE

    usage: test_sketch.py [-h] [-d DIM DIM]

    Try sketcking some mazes.
...
if __name__ == '__main__':
    ...
    maze = Maze(grid)
    Wilson.on(maze)
    sketcher = MazeSketcher(maze, title='Wilson\'s Algorithm')
    maze.sketch()
    ...
    maze = Maze(grid)
    Wilson.on(maze)
    sketcher = MazeSketcher(maze, title='Recursive Backtracker (DFS)')
    maze.sketch()

# end of test_sketch.py
