"""test_grid6.py - test the 6-connected rectangular grid
Copyright 2022 by Eric Conrad

USAGE

    usage: test_grid6.py [-h] [-d DIM DIM]

    Test the 6-connected rectangular grid.
    ...
    optional arguments:
        ...
        --alt
                          use the negatively sloped diagonals
...
import mazelib
from grid8 import Rectangular6Grid
...
if __name__ == '__main__':
    ...
    desc = 'Test the 6-connected rectangular grid.'
    ...
    rows, cols = args.dim
        ...
        # --------- TESTING STARTS HERE ----------
    print('Test 1)',
          'Wilson\'s algorithm on a 6-connected rectangular grid',
          f' (alt={args.alt})')
    kwargs = {}
    if args.alt:
        kwargs['configure_diagonal'] = -1
    grid = Rectangular6Grid(rows, cols, **kwargs)
    maze = Maze(grid)
    Wilson.on(maze)
    ...
    sketcher = MazeSketcher(maze, \
        title='Wilson\'s algorithm on a 6-connected rectangular grid'\
        + f' (alt={args.alt})')
    maze.sketch()
